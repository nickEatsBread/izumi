...
<div class="mx-auto max-w-3xl p-4 pb-24 sm:p-8">
  ...
  {#if !$watchParty}
    ...
    <p class="mt-5 rounded-lg bg-secondary/30 p-3 text-xs text-muted-foreground">Watch Together rooms are separate from Device Sync. Joining shares room presence, the host's current source, and playback controls for the current room; it does not share your history, settings or source setup.</p>
    {#if debridHost}
