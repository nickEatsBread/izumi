...
<div class="mx-auto max-w-3xl p-4 pb-24 sm:p-8">
  <div class="mb-6"><h1 class="text-2xl font-black">Watch Together</h1><p class="mt-1 text-sm text-muted-foreground">Keep playback synchronized across paired Izumi devices. Everyone resolves their own source.</p></div>
  ...
  {#if !$watchParty}
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <section class="rounded-2xl border border-border bg-secondary/30 p-5"><h2 class="text-lg font-black">Join a room</h2><p class="mb-4 mt-1 text-sm text-muted-foreground">Enter the code shown on the host device.</p><input bind:value={code} maxlength="6" placeholder="ABC234" class="mb-3 w-full rounded-lg bg-input px-4 py-3 text-center font-mono text-xl font-black uppercase tracking-[0.3em]" /><button disabled={busy} onclick={() => run(() => joinWatchParty(code))} class="w-full rounded-lg bg-secondary py-2.5 font-black hover:bg-accent">Join room</button></section>
    </div>
  ...
  {:else}
    <section class="rounded-2xl border border-theme/30 bg-theme/5 p-6 text-center">
      ...
      <p class="mt-3 text-sm text-muted-foreground">{$watchParty.role === 'host' ? 'Start any episode. Play, pause and seeking will be sent to everyone in this room.' : 'Open the source picker if prompted. Playback will align with the host after your source starts.'}</p>
