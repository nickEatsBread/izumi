...
<div class="p-4 sm:p-8">
  ...
  <div class="max-w-2xl space-y-3">
    <Toggle label="Use DNS over HTTPS" desc="Resolves hostnames via the DoH endpoint below for source extensions, addons, metadata (AniZip/Kitsu) and downloads. JVM source extensions fail closed instead of falling back to intercepted system DNS; other app requests fall back if DoH is unreachable. AniList/MAL browse and mpv playback still use the OS resolver." value={$enableDoH} onToggle={() => ($enableDoH = !$enableDoH)} />
    {#if $enableDoH}
