...
describe('DiscussAnime embed theming', () => {
  ...
  it('keeps the Deck embed on a download-free font path with touch scroll containment', () => {
    ...
    expect(loader).toContain("type: 'izumi-comments-scroll-activity'")
    expect(loader).toContain("window.addEventListener('scroll', notifyHostMotion")
    ...
    expect(lib).toContain("window.top.postMessage({type:'izumi-comments-scroll-activity'}")
    expect(panel).toContain("m?.type === 'izumi-comments-touch-scroll'")
    expect(panel).toContain("m?.type === 'izumi-comments-scroll-activity'")
    expect(panel).toContain('commentsOverlayMoving.set(true)')
    expect(panel).toContain('commentsOverlayMoving.set(false)')
