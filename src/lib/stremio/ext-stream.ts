import { normalizeTorrentCount, type TorrentResult } from '$lib/extensions/types'
...
export function extToStream(r: TorrentResult, extName: string): Stream {
  const seeders = normalizeTorrentCount(r.seeders)
  ...
  // carries the normalized value structurally for parsing and dedupe ranking — see dedupeStreams.
  const meta = [seeders ? `👤 ${seeders}` : '', gb ? `💾 ${gb}` : ''].filter(Boolean).join(' ')
  return {
    infoHash: r.hash,
    ...
    __evidence: r.evidence ? { ...r.evidence } : undefined,
