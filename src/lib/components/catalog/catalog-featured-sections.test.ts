...
describe('featured TMDB home sections', () => {
  ...
  it('uses complete provider marks, lazy remote hover media, and direct navigation', () => {
    ...
    expect(streamingRow).toContain('.brand-max .provider-fallback')
    expect(streamingRow).toContain('.brand-disney .provider-mark { width: 72%; height: 68%; }')
    expect(streamingRow).toContain('.brand-hulu .provider-mark { width: 62%; height: 54%; }')
