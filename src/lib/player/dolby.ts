...
export interface DolbyCapabilities {
  ...
  audioDevices: { id: string; name: string; encodings?: string[] }[]
  video: {
 ...
 * AC-3; E-AC3/TrueHD Atmos need HDMI/eARC. Auto is conservative when a platform cannot report the
...
export type DolbyAudioOutput = 'pcm' | 'ac3' | 'eac3' | 'truehd' | 'unknown'
export type DolbyVideoOutput = 'sdr' | 'hdr10' | 'dolby-vision' | 'unknown'
...
export function classifyVideoOutput(
  ...
  nativeDolbyVisionPath = false,
): DolbyVideoOutput {
  if (nativeDolbyVisionPath && /dolby|dovi|dvhe/i.test(`${current.videoFormat} ${current.videoProfile}`)) {
    return 'dolby-vision'
  }
