...
describe('example theme packages', () => {
  ...
  it('recreates Kindling as a featured-banner home and split series page, not a hidden-hero grid', () => {
    ...
    expect(presentation?.detail).toMatchObject({ layout: 'split', bannerHidden: false, posterWidth: 230, actionsFirst: true, coverAlign: 'end', cta: 'large', episodes: { placement: 'right', arrangement: 'list', hover: 'scale' } })
