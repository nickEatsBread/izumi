...
describe('airing countdown text', () => {
  it('uses the two largest units in compact form', () => {
    ...
    expect(compactCountdown(86400)).toBe('1d 0h')
    expect(compactCountdown(3599)).toBe('59m')
  })
  it('spells units out in long form with singulars', () => {
    ...
    expect(longCountdown(3600)).toBe('1 hr 0 mins')
    expect(longCountdown(5 * 3600 + 30)).toBe('5 hrs 0 mins')
    expect(longCountdown(59)).toBe('1 min')
    expect(longCountdown(0)).toBe('1 min')
    expect(longCountdown(-10)).toBe('1 min')
  })
})
