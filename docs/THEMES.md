...
- **Ambient colour.** While a theme stylesheet is applied, the home hero publishes `--hero-ambient-rgb` (`r g b`) on `<html>` from the current artwork, when the image can be read.
- **Protected surfaces and safe mode.** The Themes page, Theme Studio's panel and the install preview bar carry `data-theme-protected` and read a client-owned palette (`--izumi-safe-*`, written by `src/lib/theme.ts`). Engines without `@scope` remove the stylesheet while the Themes page is open. Safe mode (Ctrl/Cmd + Alt + Shift + T, or `izumi://safe-mode`) turns theme stylesheets off for the session without changing the saved theme; the Themes page reports a rejected stylesheet.
